function SA_Results = f_sensitivity_analysis_allmuscle(SA_Param)

    % 
    % f_sensitivity_analysis_allmuscle: Used to perform sensitivity analysis (All muscle in the model)
    %
    % Args:
    ...
    input_model = Model(sa_param_revise.file.model);
    muscles = input_model.getMuscles();
    muscle_num = muscles.getSize;
    ...
    for i = 1 : muscle_num
        % interest muscle
        sa_param_revise.interest.muscle_name = string(muscles.get(i - 1).getName);
